...
public:
    ChatServer(EventLoop *loop,               // 事件循环
        ...
        //初始化列表：以一个冒号开始，接着是一个以逗号分隔的数据成员列表，每个“成员变量”后边括号里放一个初始化值或者表达式
        //没有默认构造函数的类 必须用初始化列表
    {
        // 给服务器注册用户连接的创建和断开回调
        _server.setConnectionCallback(bind(&ChatServer::onConnection, this, _1)); //_1参数占位符,函数调用的时候传递参数
        // 给服务器注册用户读写事件回调
...
private:
    ...
    {
        string buf = buffer->retrieveAllAsString();  // 将buffer中的信息转换成字符串模式
